...
var colorPicker = {
	...
	runColorPicker = function runColorPicker(t) {
		...
		(document.getElementById('color_picker').style.display = 'block'),
			...
			updateColorDisplays(e.getAttribute('data-color')),
			e.setAttribute('data-color-active', !0);
	};

!(function () {
	console.log('dwqdqwd');

	for (var _t in document.getElementsByClassName('color_picker')) {
		if (!0 === isNaN(_t)) continue;
		console.log(document.getElementsByClassName('color_picker')[_t]),
			(document.getElementsByClassName('color_picker')[_t].onclick = runColorPicker);

		var _e = document.getElementsByClassName('color_picker')[_t].getAttribute('data-color');

		document.getElementsByClassName('color_picker')[_t].style.background = _e;
	}
	...
	if (
		...
		for (var _t2 = LSCustomColors[0].length - 1; _t2 >= 0; _t2--) {
			var _e2 = document.createElement('BUTTON');

			(_e2.className = 'custom_colors_preview'),
				(_e2.style.background = LSCustomColors[0][_t2]),
				_e2.setAttribute('data-custom-color', LSCustomColors[0][_t2]),
				document
					.getElementById('custom_colors_box')
					.insertBefore(_e2, document.getElementById('custom_colors_box').children[0]);
		}

		28 == LSCustomColors[0].length &&
	...
	document.getElementById('color_picker_bg').addEventListener('click', function () {
		...
		t.setAttribute(
			'data-color',
			'hsl('
				.concat(colorPicker.hue, ', ')
				.concat(colorPicker.saturation, '%, ')
				.concat(colorPicker.lightness, '%, ')
				.concat(colorPicker.alpha, ')')
		),
			(t.style.background = 'hsl('
				.concat(colorPicker.hue, ', ')
				.concat(colorPicker.saturation, '%, ')
				.concat(colorPicker.lightness, '%, ')
				.concat(colorPicker.alpha, ')')),
			t.removeAttribute('data-color-active');
	});
...
var HSLAToRGBA = function HSLAToRGBA(t, e, o, l, n) {
		...
		return (
			0 <= t && t < 60
				...
				: 240 <= t && t < 300
				? ((i = c), (a = 0), (u = r))
				: 300 <= t && t < 360 && ((i = r), (a = 0), (u = c)),
			(i = Math.round(255 * (i + s))),
			(a = Math.round(255 * (a + s))),
			(u = Math.round(255 * (u + s))),
			!0 === n
				? RGBAToHexA(i, a, u, l)
				: {
						r: i,
	...
	hexAToRGBA = function hexAToRGBA(t, e) {
		...
		return (
			...
			(r = +(r / 255).toFixed(3)),
			!0 === e
				? (console.log(+o, +l, +n, +r + ' - run'), RGBAToHSLA(+o, +l, +n, r))
				: 'rgba(' + +o + ',' + +l + ',' + +n + ',' + r + ')'
		);
	},
...
var addCustomColor = function addCustomColor() {
	...
	(e.className = 'custom_colors_preview'),
		...
		document
			.getElementById('custom_colors_box')
			.insertBefore(e, document.getElementById('custom_colors_box').children[0]),
		LSCustomColors[0].unshift(t),
	...
	document.getElementById('custom_colors_box').addEventListener('contextmenu', function (t) {
		if ('custom_colors_preview' == t.target.className) {
			...
			(e.style.display = 'block'),
				...
				(colorPicker.contextMenuElem = t.target);
		}
	});

var clearSingleCustomColor = function clearSingleCustomColor(t) {
	console.log(t);
	var e = null === t ? colorPicker.contextMenuElem : t;
	document.getElementById('custom_colors_box').removeChild(e),
		(LSCustomColors = {
			0: []
		});

	for (var _t4 in document.getElementsByClassName('custom_colors_preview')) {
		!0 !== isNaN(_t4) &&
			LSCustomColors[0].push(
				document
					.getElementsByClassName('custom_colors_preview')
					[_t4].getAttribute('data-custom-color')
			);
	}
...
var clearSingleCustomColorTouch = function clearSingleCustomColorTouch(t) {
	if ('custom_colors_preview' == t.target.className) {
		...
		e < 200 && e > 0
			? clearSingleCustomColor(t.target)
...
var colorSliderHandler = function colorSliderHandler(t) {
	...
	var c = 'hsl('
		...
		.concat(colorPicker.lightness, '%, ')
		.concat(colorPicker.alpha, ')');
	document.getElementById('color_picked_preview').children[0].setAttribute('fill', c),
		(document.getElementById('saturation').children[1].attributes[1].nodeValue = 'hsl('.concat(
			r,
			', 100%, 50%)'
		)),
...
document.getElementById('color_slider').addEventListener('mousedown', function (t) {
	...
	document.addEventListener(
		'touchmove',
		function () {
			!0 === colorPicker.sliderStatusTouch &&
				(event.preventDefault(), colorSliderHandler(event.changedTouches[0].clientX));
		},
		{
			passive: !1
		}
...
var opacitySliderHandler = function opacitySliderHandler(t) {
	...
	var r = 'hsl('
		...
		.concat(colorPicker.lightness, '%, ')
		.concat(n, ')');
	document.getElementById('color_picked_preview').children[0].setAttribute('fill', r),
		updateColorValueInput();
};
...
document.getElementById('opacity_slider').addEventListener('mousedown', function (t) {
	...
	document.addEventListener(
		'touchmove',
		function () {
			!0 === colorPicker.opacityStatusTouch &&
				(event.preventDefault(), opacitySliderHandler(event.changedTouches[0].clientX));
		},
		{
			passive: !1
		}
...
var colorBoxHandler = function colorBoxHandler(t, e) {
	...
	n < 14 && (n = 14),
		...
		r < 14 && (r = 14),
		r > 173 && (r = 173),
		(l.attributes.y.nodeValue = r),
		(l.attributes.x.nodeValue = n);
	var c = Math.round(((n - 15) / 322) * 100),
		s = 100 - c / 2,
		i = 100 - ((r - 15) / 159) * 100;
	var a = Math.floor((i / 100) * s);
	...
	var u = 'hsl('
		...
		.concat(a, '%, ')
		.concat(colorPicker.alpha, ')');
	(document.getElementsByClassName('color_picker')[0].innerHTML = u),
		document.getElementById('color_picked_preview').children[0].setAttribute('fill', u),
		updateColorValueInput();
};
...
document.getElementById('color_box').addEventListener('mousedown', function (t) {
	...
	document.getElementById('color_box').addEventListener('touchstart', function (t) {
		(colorPicker.boxStatusTouch = !0),
			colorBoxHandler(t.changedTouches[0].clientX, t.changedTouches[0].clientY);
	}),
	document.addEventListener(
		'touchmove',
		function () {
			!0 === colorPicker.boxStatusTouch &&
...
var updateColorDisplays = function updateColorDisplays(t) {
		if ('#' == t.substring(0, 1)) t = hexAToRGBA(t, !0);
		else if ('r' == t.substring(0, 1)) {
			var _e3 = t.match(/[.?\d]+/g);

			(_e3[3] = null == _e3[3] ? 1 : _e3[3]),
				(t = RGBAToHSLA(_e3[0], _e3[1], _e3[2], _e3[3]));
		} else {
			var _e4 = t.match(/[.?\d]+/g);

			(_e4[3] = null == _e4[3] ? 1 : _e4[3]),
				(t = {
					h: _e4[0],
					s: _e4[1],
					l: _e4[2],
		...
		(colorPicker.hue = t.h),
			(colorPicker.saturation = t.s),
			(colorPicker.lightness = t.l),
			(colorPicker.alpha = t.a),
			updateColorValueInput(),
			document
				.getElementById('color_picked_preview')
				.children[0].setAttribute(
					'fill',
					'hsl('.concat(t.h, ', ').concat(t.s, '%, ').concat(t.l, '%, ').concat(t.a)
				),
			(document.getElementById(
				'saturation'
			).children[1].attributes[1].nodeValue = 'hsl('.concat(t.h, ', 100%, 50%)'));
		var e = document.getElementById('box_dragger');
		var o, l;
		(l = 1.59 * (100 - (t.l / (100 - t.s / 2)) * 100) + 14),
			(o = 3.22 * t.s + 14),
	...
	updateColorValueInput = function updateColorValueInput() {
		if ('HEXA' == colorPicker.colorTypeStatus) {
			...
			(document.getElementsByClassName('rgba_input')[0].value = _t5.r),
				(document.getElementsByClassName('rgba_input')[1].value = _t5.g),
				(document.getElementsByClassName('rgba_input')[2].value = _t5.b),
				(document.getElementsByClassName('rgba_input')[3].value = _t5.a);
		} else
			(document.getElementsByClassName('hsla_input')[0].value = colorPicker.hue),
				(document.getElementsByClassName('hsla_input')[1].value = colorPicker.saturation),
				(document.getElementsByClassName('hsla_input')[2].value = colorPicker.lightness),
