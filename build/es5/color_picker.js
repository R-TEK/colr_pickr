...
!function () {
  ...
  for (var _t in document.getElementsByClassName("color_picker")) {
    ...
    console.log(document.getElementsByClassName("color_picker")[_t]), document.getElementsByClassName("color_picker")[_t].onclick = runColorPicker;
  ...
  if (e.id = "color_picker_bg", document.getElementsByTagName("BODY")[0].appendChild(e), null === localStorage.getItem("custom_colors")) localStorage.setItem("custom_colors", '{"0": []}');else {
    ...
    for (var _t2 = LSCustomColors[0].length - 1; _t2 >= 0; _t2--) {
      ...
      _e2.className = "custom_colors_preview", _e2.style.background = LSCustomColors[0][_t2], _e2.setAttribute("data-custom-color", LSCustomColors[0][_t2]), document.getElementById("custom_colors_box").insertBefore(_e2, document.getElementById("custom_colors_box").children[0]);
    }
  }
...
var HSLAToRGBA = function HSLAToRGBA(t, e, o, l, n) {
  ...
  var r = (1 - Math.abs(2 * o - 1)) * e,
      ...
      a = 0,
      u = 0;
  return 0 <= t && t < 60 ? (i = r, a = s, u = 0) : 60 <= t && t < 120 ? (i = s, a = r, u = 0) : 120 <= t && t < 180 ? (i = 0, a = r, u = s) : 180 <= t && t < 240 ? (i = 0, a = s, u = r) : 240 <= t && t < 300 ? (i = s, a = 0, u = r) : 300 <= t && t < 360 && (i = r, a = 0, u = s), i = Math.round(255 * (i + c)), a = Math.round(255 * (a + c)), u = Math.round(255 * (u + c)), !0 === n ? RGBAToHexA(i, a, u, l) : {
    r: i,
    g: a,
    b: u,
    a: l
  ...
  var n = Math.min(t, e, o),
      ...
      i = 0,
      a = 0;
  return c = 0 == s ? 0 : r == t ? (e - o) / s % 6 : r == e ? (o - t) / s + 2 : (t - e) / s + 4, c = Math.round(60 * c), c < 0 && (c += 360), a = (r + n) / 2, i = 0 == s ? 0 : s / (1 - Math.abs(2 * a - 1)), i = +(100 * i).toFixed(1), a = +(100 * a).toFixed(1), console.log(c, i, a, l), {
    h: c,
    s: i,
    l: a,
    a: l
...
var colorSliderHandler = function colorSliderHandler(t) {
  ...
  colorPicker.hue = r;
  var s = "hsl(".concat(r, ", ").concat(colorPicker.saturation, "%, ").concat(colorPicker.lightness, "%, ").concat(colorPicker.alpha, ")");
  document.getElementById("color_picked_preview").children[0].setAttribute("fill", s), document.getElementById("saturation").children[1].attributes[1].nodeValue = "hsl(".concat(r, ", 100%, 50%)"), updateColorValueInput();
};
...
var colorBoxHandler = function colorBoxHandler(t, e) {
  ...
  var s = Math.round((n - 15) / 322 * 100),
      c = 100 - s / 2,
      i = 100 - (r - 15) / 159 * 100,
      a = Math.round(i / 100 * c);
  colorPicker.saturation = s, colorPicker.lightness = a;
  var u = "hsl(".concat(colorPicker.hue, ", ").concat(s, "%, ").concat(a, "%, ").concat(colorPicker.alpha, ")");
  document.getElementsByClassName("color_picker")[0].innerHTML = u, document.getElementById("color_picked_preview").children[0].setAttribute("fill", u), updateColorValueInput();
};
...
var updateColorDisplays = function updateColorDisplays(t) {
  if ("#" == t.substring(0, 1)) t = hexAToRGBA(t, !0);else if ("r" == t.substring(0, 1)) {
    ...
    _e3[3] = null == _e3[3] ? 1 : _e3[3], t = RGBAToHSLA(_e3[0], _e3[1], _e3[2], _e3[3]);
  } else {
    var _e4 = t.match(/[.?\d]+/g);

    _e4[3] = null == _e4[3] ? 1 : _e4[3], t = {
      h: _e4[0],
      s: _e4[1],
      l: _e4[2],
      a: _e4[3]
  ...
  var s = document.getElementById("opacity_slider_dragger");
  var c = 100 * t.a * 2.66 + 11;
  s.attributes.x.nodeValue = c;
},
