...
!(function () {
	...
	if (
		...
		for (let t = LSCustomColors[0].length - 1; t >= 0; t--) {
			let e = document.createElement('BUTTON');
			(e.className = 'custom_colors_preview'),
				(e.style.background = LSCustomColors[0][t]),
				e.setAttribute('data-custom-color', LSCustomColors[0][t]),
...
let HSLAToRGBA = function (t, e, o, l, n) {
		(e /= 100), (o /= 100);
		...
		return (
			...
			!0 === n ? RGBAToHexA(i, a, u, l) : { r: i, g: a, b: u, a: l }
		);
	},
	RGBAToHSLA = function (t, e, o, l) {
...
document.getElementById('switch_color_type').addEventListener('click', function () {
	...
	document.querySelectorAll('.rgba_input').forEach((t) => {
		t.addEventListener('change', function () {
			...
			updateColorDisplays(`rgba(${t[0].value}, ${t[1].value}, ${t[2].value}, ${t[3].value})`);
		});
	}),
	document.querySelectorAll('.hsla_input').forEach((t) => {
		t.addEventListener('change', function () {
			...
			if (t[3].value > 1) throw 'Value must be equal to or below 1';
			updateColorDisplays(
				`hsl(${t[0].value}, ${t[1].value}%, ${t[2].value}%, ${t[3].value})`
			);
		});
	}),
...
let addCustomColor = function () {
	27 == LSCustomColors[0].length &&
		(document.getElementById('custom_colors_add').style.display = 'none');
	const t = `hsl(${colorPicker.hue}, ${colorPicker.saturation}%, ${colorPicker.lightness}%, ${colorPicker.alpha})`;
	let e = document.createElement('BUTTON');
	(e.className = 'custom_colors_preview'),
...
let clearSingleCustomColor = function (t) {
	const e = void 0 === t ? colorPicker.contextMenuElem : t;
	document.getElementById('custom_colors_box').removeChild(e), (LSCustomColors = { 0: [] });
...
let clearSingleCustomColorTouch = function (t) {
	if ('custom_colors_preview' == t.target.className) {
		const e = new Date().getTime() - colorPicker.doubleTapTime;
		e < 200 && e > 0
...
let colorSliderHandler = function (t) {
	...
	const n = ((l - 11) / 266) * 100,
		r = Math.round(359 - 3.59 * n);
	colorPicker.hue = r;
	const c = `hsl(${r}, ${colorPicker.saturation}%, ${colorPicker.lightness}%, ${colorPicker.alpha})`;
	document.getElementById('color_picked_preview').children[0].setAttribute('fill', c),
		(document.getElementById(
			'saturation'
		).children[1].attributes[1].nodeValue = `hsl(${r}, 100%, 50%)`),
...
let colorBoxHandler = function (t, e, o) {
	...
	r < 14 && (r = 14),
		...
		c < 14 && (c = 14),
		c > 173 && (c = 173),
		(n.attributes.y.nodeValue = c),
		(n.attributes.x.nodeValue = r);
	const s = Math.round(((r - 15) / 322) * 100),
		i = 100 - s / 2,
		a = 100 - ((c - 15) / 159) * 100;
	let u = Math.floor((a / 100) * i);
	(colorPicker.saturation = s), (colorPicker.lightness = u);
	const d = `hsl(${colorPicker.hue}, ${s}%, ${u}%, ${colorPicker.alpha})`;
	document.getElementById('color_picked_preview').children[0].setAttribute('fill', d),
		updateColorValueInput();
};
document.getElementById('color_box').addEventListener('mousedown', function (t) {
	...
	document.getElementById('color_box').addEventListener('touchstart', function (t) {
		(colorPicker.boxStatusTouch = !0),
			colorBoxHandler(t.changedTouches[0].clientX, t.changedTouches[0].clientY, !0);
	}),
	document.addEventListener(
		'touchmove',
		function () {
...
let updateColorDisplays = function (t) {
		if ('#' == t.substring(0, 1)) t = hexAToRGBA(t, !0);
		else if ('r' == t.substring(0, 1)) {
			const e = t.match(/[.?\d]+/g);
			(e[3] = null == e[3] ? 1 : e[3]), (t = RGBAToHSLA(e[0], e[1], e[2], e[3]));
		} else {
			const e = t.match(/[.?\d]+/g);
		...
		(colorPicker.hue = t.h),
			...
			document
				...
				.children[0].setAttribute('fill', `hsl(${t.h}, ${t.s}%, ${t.l}%, ${t.a}`),
			(document.getElementById(
				'saturation'
			).children[1].attributes[1].nodeValue = `hsl(${t.h}, 100%, 50%)`);
		...
		const n = document.getElementById('color_slider_dragger');
		let r = 2.66 * (100 - (t.h / 359) * 100) + 11;
		n.attributes.x.nodeValue = r;
		const c = document.getElementById('opacity_slider_dragger');
		let s = 100 * t.a * 2.66 + 11;
